...
| `model`           | No       | `inherit` | `haiku`, `sonnet`, `opus`, `fable`, full ID, or `inherit` |
...
Coverage per domain is computed by `bin/check-patterns-coverage.sh` — it diffs
each `check-*` contract's `## Categories` table against the categories its
`patterns.sh`/`patterns.py` actually emit (the quoted slug literals in the TSV
`category` field) and prints per-domain and overall percentages. Run it to
refresh the figures below; add `--strict [THRESHOLD]` (default 80) to fail
non-zero when overall coverage regresses, so CI can gate it.
As of the latest run: 26/31 categories (83%) are covered by the pre-scans.
Five domains have 100% deterministic coverage (ai-config, security,
code-health, deadlinks, staleness) and could potentially downgrade to haiku
for the LLM confirmation pass.
