...
| ------------------------ | -------------------------------------------------------------------------------------------------------- |
| `unreaped-subprocess`    | Subprocess spawn site with no obvious reap — Swift `Process()`, Python `Popen(` / `subprocess.Popen(`, Node `spawn/spawnSync/exec/execFile/execFileSync/execSync(`, Go `exec.Command(`, Rust `Command::new(`, Bash a trailing `&` background job (excluding `&&`, `>&` fd-dup and a `&` inside a trailing comment) |
| `terminate-without-kill` | A `.terminate()` (Swift/Python/JS) / `syscall.SIGTERM` (Go, excluding a qualified `.Notify(` registration — #871) / `SIGTERM` (Rust) / `kill -TERM` \| `-15` \| `-s TERM` (Bash) send site — flagged so the LLM can confirm there is no SIGKILL escalation and no final wait |
