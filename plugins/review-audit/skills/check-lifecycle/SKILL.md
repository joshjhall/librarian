...
| `terminate-without-kill` | A `.terminate()` (Swift/Python/JS) / `syscall.SIGTERM` (Go, excluding a `signal.Notify` registration — #871) / `SIGTERM` (Rust) / `kill -TERM` \| `-15` \| `-s TERM` (Bash) send site — flagged so the LLM can confirm there is no SIGKILL escalation and no final wait |
