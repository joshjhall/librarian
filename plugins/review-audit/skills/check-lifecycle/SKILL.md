...
| `terminate-without-kill` | A `.terminate()` (Swift/Python/JS) / `os.Interrupt` (Go) / `SIGTERM` (Rust) / `kill -TERM` \| `-15` \| `-s TERM` (Bash) send site — flagged so the LLM can confirm there is no SIGKILL escalation and no final wait |
