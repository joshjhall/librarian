...
| ------------------------ | -------------------------------------------------------------------------------------------------------- |
| `unreaped-subprocess`    | Subprocess spawn site with no obvious reap — Swift `Process()`, Python `Popen(` / `subprocess.Popen(`, Node `spawn/spawnSync/exec/execFile/execFileSync/execSync(`, Go `exec.Command(`, Rust `Command::new(`, Bash a trailing `&` background job (excluding `&&`, `>&` fd-dup and assignment-shaped lines) |
