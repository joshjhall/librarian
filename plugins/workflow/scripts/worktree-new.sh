...
command echo "  tmux new-session -d -s golem-$N -c \"$root/$wt\" -e GOLEM_ID=golem-$N \"claude --permission-mode auto '/workflow:next-issue $N --autonomous' ; claude --permission-mode auto '/workflow:ship-issue --autonomous'\""
