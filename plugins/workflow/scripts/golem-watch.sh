...
("$watch" --stream-panes 2>/dev/null | command sed -u 's/^/[pane] /') &
pane_pid=$!
...
cleanup_pane() {
    command pkill -P "$pane_pid" 2>/dev/null || true
    command kill "$pane_pid" 2>/dev/null || true
}
trap cleanup_pane EXIT INT TERM
"$watch" --stream 2>/dev/null | command sed -u 's/^/[feed] /'
