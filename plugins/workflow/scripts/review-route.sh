...
#     forces `full` (R6); it can never produce `cheap`.
...
#         -> route          full | cheap
#            rule           the deciding rule (R0-empty … R7-doc-only)
...
#
# ---------------------------------------------------------------------------
# WHY R5 EXISTS: config IS security-relevant, per the normative table.
#
# Cycles 1, 3, 5 and 6 of this feature's own review each found the same bug in a
# new disguise: some file class that the harness treats as security-relevant was
# classified here as inert and routed cheap. CI workflows (#1), database schemas
# (#3), then dependency manifests and lockfiles (#6) — package.json,
# pnpm-lock.yaml, requirements.txt, Cargo.toml, pyproject.toml. A dependency bump
# is the canonical supply-chain diff shape, and it was routing around `security`
# entirely while staying eligible to return clean: true.
#
# Four instances of one class means the SHAPE is wrong, not that a fifth
# carve-out is missing. The root contradiction is one line in
# workflow.src/74-narrowing.js, the normative source:
#
#     security:    ['source', 'database', 'config', 'ci', 'docker'],
#     correctness: ['source', 'database', 'config', 'ci', 'docker'],
#
# `config` is listed. The harness re-runs `security` on a config-only delta
# precisely because a config file carries secrets, permissions, pinned versions
# and dependency hashes. This script routed `config` cheap anyway — so every
# carve-out added above (CI paths, compose files, migrations, schemas) was
# patching one symptom of that single disagreement, and the list could never be
# complete: `*.json` alone spans package manifests, lockfiles, tsconfig, IAM
# policies and inert fixtures, and no basename list distinguishes them.
#
# So the cheap path is now DOC-ONLY. Config files force `full`, which makes this
# classifier agree with DIMENSION_RELEVANT_TYPES by construction rather than by
# an ever-growing exception list. The carve-out arms above are kept — they are
# now redundant for routing (config forces full regardless), but they still
# classify correctly for the emitted counts, and deleting them would re-open the
# hole the moment someone loosens R5.
#
# COST: a `config`-only diff (a JSON fixture bump) no longer routes cheap. That
# is the intended trade. The measured win came from doc-only diffs — prose is
# this repo's largest and fastest-churning surface (#589) — and buying a
# narrower optimization with a fail-open security hole is not a trade this
# feature is willing to make.
#
# Note R7 is the ONLY rule that yields `cheap`, and it is last: every fail-safe
...
#                                     (R6). Default: 2000. A 5,000-line docs
...
classify() {
    ...
    case "$_cr_base" in
        *.py | *.js | *.jsx | *.mjs | *.cjs | *.ts | *.tsx | *.rs | *.go | *.sh | *.bash | *.swift)
            ...
            ;;
        # `.txt` is deliberately NOT here. It was borrowed from
        # check-lifecycle/patterns.sh, which skips it for a different purpose
        # (excluding non-source files from a lifecycle scan, not deciding
        # whether security review runs). `requirements.txt` is a Python
        # dependency pin file — the canonical supply-chain diff — and calling it
        # prose routed it cheap. A `.txt` is prose often enough to be tempting
        # and a manifest often enough to be dangerous, so it falls to `unknown`
        # and forces `full`, per this script's own fail-safe direction.
...
_has_unsurfaceable_category() {
    ...
    # GLOBBING OFF while we split. `for x in $list` performs word-splitting AND
    # pathname expansion, and only the first is wanted here. Reproduced: with a
    # file `decomposition-seam-XYZ` in the cwd, the token `decomposition-seam-*`
    # is silently replaced by that FILENAME before the `case` ever sees it — so
    # the documented "exact-matched token by token, never substring-matched"
    # guarantee was not actually held, and a category that failed to match this
    # way would let an unsurfaceable pre-scan row route cheap. The script runs
    # from the repo root during a real cycle, so the cwd is full of candidates.
    # `set -f` is bash-3.2 clean and needs no arrays; restored immediately after.
    set -f
...
cmd_check() {
    ...
    elif [ "$_n_config" -gt 0 ]; then
        # `config` is security-relevant per DIMENSION_RELEVANT_TYPES — see the
        # header. This is what makes the four carve-outs above unnecessary as
        # routing logic, and what closes the dependency-manifest hole
        # (package.json, pnpm-lock.yaml, requirements.txt, Cargo.toml) for good.
        _route="full"
        _rule="R5-config"
        _reason="config-file-present"
    elif [ -n "$_diff_lines" ] && [ "$_diff_lines" -gt "$_max_lines" ]; then
        _route="full"
        _rule="R6-max-lines"
        _reason="diff-over-line-ceiling"
    else
        # Reached only when every file classified DOC and no fail-safe fired.
        # This is the ONLY path to `cheap`.
        _route="cheap"
        _rule="R7-doc-only"
        _reason="doc-only"
    fi
