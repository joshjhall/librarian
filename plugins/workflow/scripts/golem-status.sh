...
GREP="$(_bin grep)"
HEAD="$(_bin head)"
# shellcheck disable=SC2034  # $RM is read by the sourced signals fragment (#800)
RM="$(_bin rm)"
SLEEP="$(_bin sleep)"
STAT="$(_bin stat)"
TAIL="$(_bin tail)"
...
# $scrape / $ctxbudget are read by golem-status-signals.sh, sourced just below —
# cross-file, so the linter (one file at a time) sees them as unused here. They
# are assigned HERE, not there, because they are this script's own path consts
# and the fragment is deliberately the consumer, not the owner.
# shellcheck disable=SC2034  # consumed by the sourced signals fragment
scrape="$SCRIPT_DIR/golem-token-scrape.sh"
# shellcheck disable=SC2034  # consumed by the sourced signals fragment
ctxbudget="$SCRIPT_DIR/context-budget.sh"
# The two PER-GOLEM SIGNAL families — each a cache reader plus its render half
# (#800). Sourced HERE, above both render functions and above the main-guard at
# the foot of this file, so the verbose render, the --checkpoint render, and a
# test that sources this script to unit-test helpers all resolve them. It reads
# $scrape / $ctxbudget / $root / $status_dir and the time helpers below, so it
# must come after those assignments.
...
render_status() {
    ...
    # The two per-golem signal sections (#800). Both live in
    # golem-status-signals.sh, each beside the cache reader it displays; they are
    # called here, in this order and at this position, so the verbose render's
    # section order and text are unchanged by the extraction.
    render_token_block
