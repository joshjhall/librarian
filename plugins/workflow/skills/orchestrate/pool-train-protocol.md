...
Phase T's step-by-step moved to **`train-protocol.md`** in #973, when this file
passed its prose budget; this one carries Phase P.
...
1. **Refill decision.** If `pool.queue == "accepting"` **and** a slot is
   ...
   Stage the harness (`${CLAUDE_PLUGIN_ROOT}/scripts/harness-stage.sh stage
   orchestrate`, #973) and invoke the Workflow tool on its `path=` with:
...
**Companion file**: `train-protocol.md` (load before this phase) carries the
full integration-train protocol — batch authorization, merge order, the
merge → rebase → merge loop, and the CI-subset policy.

The pool above feeds work in; the train lands it. Land a **batch** of
already-green, already-approved PRs with **one up-front authorization** instead
of one human gate per merge/rebase/push.
