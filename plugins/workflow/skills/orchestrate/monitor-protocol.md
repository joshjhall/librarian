...
**Brokering a human answer back to a blocked golem is a separate file** —
`gate-brokering.md` (#973). This one surfaces gates; that one resolves them.
...
1. **Stage the harness** (`${CLAUDE_PLUGIN_ROOT}/scripts/harness-stage.sh stage
   orchestrate`, #973) and **invoke the Workflow tool** on its `path=`
   with:
...
**Answering a blocked golem is `gate-brokering.md`** (#973). Once this sweep
surfaces a `gate`, `escalation`, or `dead-end` line, the protocol for relaying a
human decision back into the golem — the central inbox (#227), the directed
`tmux send-keys` plan-gate broker, multi-question forms, and the data-only
invariant — lives in that companion. Load it at the point you have a decision to
deliver.
