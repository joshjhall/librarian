...
// the size row.
//
// THE OTHER TWO LEVERS ARE CLOSED ON MEASUREMENT (#718, #503 AC3). #503 AC3
// proposed that much of this file's bulk was accreted reviewer-prompt prose,
// making a trim "plausibly the largest real win". Measured, it is not:
//
//   - PROMPT PROSE. String literals are 258 of 1084 non-comment lines (24%) and
//     16,613 of 37,898 non-comment chars (44%). The decisive number is the time
//     series, not the snapshot: at 6e2aafd — the 649-raw-line revision near the
//     issue's filing — this file measured the SAME 24%/44%. The +128% growth was
//     therefore PROPORTIONAL, prompt and control flow together, and git log
//     attributes it to fourteen behavioral commits (#711, #617, #565, #556/#557,
//     #553, #492, …). There is no prose surplus to reclaim.
//   - SHARED LOGIC ACROSS HARNESSES. A 6-line-window sweep across all six
//     harnesses finds 142 duplicated lines here (7%) — `attempt`,
//     `stableStringify`, the budget-degradation guard. Real, but note what #806
//     did and did not change: fragments are shareable WITHIN this harness, while
//     the generated artifact still cannot `import`, so nothing can be shared
//     ACROSS harnesses. That is the BUDGET_FLOOR class, gated by
//     tests/lint-skills-agents.sh rather than eliminated.
//
// So the size row is a RESULT, not an open to-do: three levers, each measured,
// the one that applies applied.
//
...
//                                          // (#550). 'cheap' ⇒ a doc/config-only
//                                          // diff: run scope-drift ALONE. Anything
//                                          // other than the exact string 'cheap'
//                                          // means 'full', so a malformed value
//                                          // fails safe toward the full fan-out
