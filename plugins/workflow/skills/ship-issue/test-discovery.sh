...
#              prefix is TWO OR MORE digits (#894): tests/golem-scripts/ has
#              grown past 99, so an exactly-two-digit glob cannot see
#              100-mode-check.sh or 110-tracks-runbook.sh and reports their
#              sources untested while the tests sit in the tree. The `*` sits
#              between the digits and the hyphen, so it widens only the PREFIX
#              — the name half of the glob is still fully anchored.
#
...
#
# Its numeric prefix IS `[0-9][0-9]*-` (#894), and that is not a loosening of the
# restriction this function exists to hold. The restriction is on the CANDIDATE
# half — no `-*`/`_*` suffix forms, because a wildcard on a short generic token
# was measured to match `bin/ruff-version.sh` against
# `tests/release/10-version-utils.sh`. This `*` sits between the digits and the
# hyphen and can only ever consume more DIGITS before a literal `-${cand}.sh`,
# so it admits `100-` and `110-` fragments without touching the candidate anchor.
