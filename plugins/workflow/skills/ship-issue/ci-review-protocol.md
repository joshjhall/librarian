...
post-creation steps, which run after the PR is created and the auto-merge fast
path is NOT taken. The sequence has two phases, split across two files in #973
when this one passed its prose budget:

1. **CI monitoring + failure triage** — **`ci-monitor-protocol.md`**. Load it
   while CI is red or still running.
1. **The multi-cycle PR review loop**, finding classification, and filing
   deferred findings — this file, below. It assumes green CI.

Environment variables (`REVIEW_MAX_CYCLES`, `REVIEW_MAX_ATTEMPTS`, and the
`LIBRARIAN_CI_*` set the monitor phase reads) are defined in `ship-protocol.md`
§ Environment Variables.

## Monitor CI and remediate failures

Moved to **`ci-monitor-protocol.md`** in this skill directory (#973), which
carries the whole CI-monitoring and failure-triage phase: the wait loop and its
timeout ladder, infra-flake retries, and the `ci-fixer` harness dispatch.

**Load it when CI is red or still running.** The review loop below assumes green
CI and does not start until it is — so the two halves are sequential phases,
which is the seam this split follows.
...
c. **Stage the harness**, then **Invoke the `Workflow` tool** on the `path=` it
prints — **already opted in** (`ship-protocol.md` § *Workflow authority*, #637).
Staging is required because the `Workflow` tool refuses a `scriptPath` outside
the session cwd, which the installed plugin root always is (#973):
...
Run it bare and read `path=`; never `eval "$(…)"` (#815). Pass that path as
`scriptPath`, with:
...
**Graceful degradation — mechanical failure only (#637), narrowed by exit code
(#973)**: the disposition is decided by `harness-stage.sh`'s exit status, exactly
as in `pre-ship-validation.md` Step 3.5 item 6:

- **Exit 4** (owning plugin not installed), or the `Workflow` tool errors on a
  successfully-staged path → **skip** with a note ("Multi-cycle review skipped
  (harness not available)"), carry it into the completion summary as
  `Review status: skipped: {reason}`, and proceed to labeling. Review never
  blocks shipping for this class.
- **Exit 3** (unresolvable or unstageable — broken install, override pointing
  nowhere, unwritable cwd) → **do not skip.** Surface
  `Review status: unavailable: {reason}` with the probe list, and stop for a
  human. An unreachable harness is a broken environment, not an absent tool.

The un-narrowed clause keyed on a path that existed on no tree, so it was
permanently satisfied and the loop was skipped on every run (#973). A skip that
fires every time is not graceful degradation.
