...
**Companion file**: the full autonomy rule lives in `autonomous-mode.md` in this
skill directory — load it whenever the run is autonomous (or to decide whether
it is). It carries the toggle definition, the gate-skipping vs plan-skipping
split, the `--plan-gate` / `--force-auto` overrides, the `severity/critical`
second-consent (`FORCE_AUTO_CRITICAL=1`), and the shipping handoff. The summary
below is the operational gist; `autonomous-mode.md` is authoritative.
...
opt-in. Autonomy splits into **two independent sub-behaviors**:

- **Gate-skipping (always on when autonomous).** Do NOT call any
  `AskUserQuestion`. Every human gate below — issue acceptance, branch-freshness,
  drift, shipping mode, CI waits — takes its documented default with no
  interactive tool call.
- **Plan-skipping (conditional).** The plan checkpoint
  (`EnterPlanMode`/`ExitPlanMode`) is skipped **only** for `effort/trivial` or
  `effort/small` issues that are **not** `severity/critical` (fully autonomous —
  `plan_gated: false`). Every other autonomous run (`effort/medium`/`large`,
  `severity/critical`, or no `effort/*` label) is **plan-gated**
  (`plan_gated: true`): it builds the plan and STOPS at `ExitPlanMode` for human
  approval, then continues autonomously through implement → review → push/PR.
  Overrides: `--plan-gate` forces the checkpoint on a small issue; `--force-auto`
  forces full plan-skipping on a medium+/critical one (on a `severity/critical`
