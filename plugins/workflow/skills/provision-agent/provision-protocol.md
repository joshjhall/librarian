   ...
   services:
     agent01:
       ...
       environment:
         ...
         REVIEW_CONVERGENCE_SURFACE_RATIO: "${REVIEW_CONVERGENCE_SURFACE_RATIO:-50}"
...
1. **Write `.worktrees/agent-entrypoint.sh`** — a wrapper script that verifies
   ...
   # Percent of the previous cycle's reviewed surface at which a zero-finding
   # cycle counts as real convergence (review-convergence.sh). Host and
   # bare-linux golems inherit this from the environment for free; a container
   # golem only sees it if compose passes it through, hence both sites (#621).
