...
| `golem-transcript-liveness.sh <worktree>` | Classify a golem working/background/idle/errored from its newest transcript's top-level `stop_reason`/`isApiErrorMessage` plus the background-work registry — the TTY-free headless fallback tier in `golem-gate-watch.sh`'s liveness sweep (#248, #949). The registry arm is the one signal that reaches a Mode-3 container golem |
...
| `token-attribute.sh debt\|floor\|growth\|prefix\|bash-class\|attachments` | The transcript side of the same question (#788): the gateway says HOW MUCH, this says WHERE it went — per-tool re-read debt (result size x turns resident), context-floor decomposition, per-decile growth within a session, spawn prefix distribution, Bash read-vs-mutate, attachment volume. Emits the same TSV contract keyed on `(window_start, model)` so its rows join against `token-report.sh window` — **pass the same `--since`/`--until`**, since an unscoped scan spans the whole transcript history and joins against nothing (it warns when you omit them). Every aggregation is grouped per model, so a multi-model window never collapses onto the majority one. Python 3.11+ with NO bash fallback by design — exits `77` rather than degrade. Four transcript-parsing traps (dedup by `message.id`, union blocks across its lines, thinking tokens from `usage`, UTC normalization) are pinned by mutation-verified fixtures in `tests/validate-token-attribute.sh`. Ships as three files: the CLI plus `token_attribute_engine.py` (transcript reading, where the traps live) and `token_attribute_reports.py` (the six reports) |
...
| `LIBRARIAN_REVIEW_ROUTE` | `auto` | `review-route.sh` | Set `full` to disable doc-only review routing (#550) and always run the complete fan-out. Any value but `auto` reads as `full`, so a typo disables the optimization rather than enabling it |
| `LIBRARIAN_REVIEW_ROUTE_MAX_LINES` | `2000` | `review-route.sh` | Diff-line ceiling above which the cheap path is refused even for a doc-only diff. Can only ever force `full`, never produce `cheap` |
