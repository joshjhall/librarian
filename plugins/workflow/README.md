...
| `token-report.sh window\|compare\|reconcile` | Per-model token/cost measurement over the Bifrost gateway's aggregate `/api/logs/stats`, with a reconciliation guard that turns a silently-ignored filter param into a loud failure (#781). Emits the TSV baseline/compare contract; exits `77` when the gateway is unreachable |
...
| `TOKEN_REPORT_RECONCILE_PCT` | `0.5` | Reconciliation tolerance, percent of the unfiltered request total. Headroom rather than an observed need — a complete model list reconciles exactly (28,702 vs 28,702) — kept far below the N-fold gap a dropped `models=` filter opens (measured 401,828 vs 28,702) |
