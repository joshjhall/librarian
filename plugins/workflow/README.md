...
| `LIBRARIAN_REVIEW_ROUTE` | `auto` | `review-route.sh` | Set `full` to disable doc/config-only review routing (#550) and always run the complete fan-out. Any value but `auto` reads as `full`, so a typo disables the optimization rather than enabling it. A routed cycle runs `scope-drift` alone and is complete-by-design — it sets neither `budget_exhausted` nor `dimensions_skipped`, so it can still return `clean`; safety rests on the classifier, never the reviewers |
