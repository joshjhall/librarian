...
| `review-route.sh check …` | Decide whether a review cycle needs the full 7-agent fan-out or the cheap doc/config-only path — an ordered rule list whose every ambiguity resolves to `full` (#550) |
...
| `LIBRARIAN_REVIEW_ROUTE` | `auto` | `review-route.sh` | Set `full` to disable doc-only review routing (#550) and always run the complete fan-out. Any value but `auto` reads as `full`, so a typo disables the optimization rather than enabling it. A routed cycle runs the dimensions whose `DIMENSION_RELEVANT_TYPES` entry claims `docs` (post-#551: `decomposition`) plus `scope-drift`, and is complete-by-design — it sets neither `budget_exhausted` nor `dimensions_skipped`, so it can still return `clean`; safety rests on the classifier, never the reviewers |
