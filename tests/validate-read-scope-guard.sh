...
# shellcheck disable=SC2034  # consumed by the sourced fixtures/fragments, not by this file
{
    ...
    # `${arr[@]/#/-u}` expansion (bash-3.2 clean) yields one arg per var.
...
run_fragment_test test_converted_worktree_still_degrades_loudly "#506: converted worktree degrades LOUDLY (not silently)"
...
run_fragment_test test_nojq_truncated_cwd_fails_open_loudly "no-jq: truncated CWD fails open LOUDLY"
