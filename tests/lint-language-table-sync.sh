...
LANG_REPORT_RC=0
LANG_REPORT="$(
    command python3 - "$LANG_TABLE_ROOT" <<'PY' 2>&1
import os
import re
import sys
...
test_analyzer_ran_to_completion() {
    if [ "$LANG_REPORT_RC" -ne 0 ]; then
        _fail "the python3 analyzer crashed (exit $LANG_REPORT_RC) — this gate checked NOTHING" \
            "Fix the analyzer, not the subject: the assertions below read a report that was never produced." \
            "${LANG_REPORT:-(no output captured)}"
    fi
}
