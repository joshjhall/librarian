...
run_test test_gitlink_poison_denies_leak "#506: gitlink-rewrite decoy still DENIES a leak"
run_test test_gitlink_poison_core_worktree_still_denies "#506: gitlink + core.worktree redirect still DENIES (CRITICAL)"
run_test test_gitlink_poison_core_bare_still_denies "#506: gitlink + core.bare poison still DENIES (CRITICAL)"
run_test test_gitlink_initdir_variant_denies "#506: git-init-the-worktree DIR-variant disguise DENIED"
run_test test_gitlink_poison_nested_cwd_denies "#506: poisoned session from a nested cwd still DENIES"
run_test test_gitlink_poison_allows_own_worktree "#506: poisoned worktree's own path still allowed"
run_test test_gitlink_poison_plus_registry_delete_denies "#506: poison + registry delete still DENIES (Bypass 1)"
run_test test_gitlink_poison_nearer_decoy_denies "#506: nearer-decoy still DENIES (Bypass 2, outermost wins)"
run_test test_gitlink_write_denied "#506: disarming write to \$WT/.git is DENIED (FIX B)"
run_test test_gitlink_write_slash_variants_denied "#506: slash-variant \$WT//.git / \$WT/.git/ write DENIED (HIGH)"
run_test test_gitlink_sgd_main_not_misdetected "#506: genuine separate-git-dir main not misdetected"
run_test test_gitlink_nojq_denies_leak "#506: no-jq path still denies a poisoned leak"
run_test test_gitlink_nojq_denies_write "#506: no-jq path still denies the gitlink write"
