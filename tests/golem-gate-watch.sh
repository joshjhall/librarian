...
run_fragment_test test_pane_multi_question_form_scrolled_off_top "pane_is_multi_question_form: a tab bar scrolled off the VISIBLE pane is detected via its own scrollback read (#1010)"
run_fragment_test test_pane_multi_question_form_scrolled_off_top_prose "pane_is_multi_question_form: glyph-bearing prose in SCROLLBACK does not fake a form (#1010)"
run_fragment_test test_pane_multi_question_form_scrollback_window "pane_is_multi_question_form: scrollback depth + GOLEM_PANE_SCROLLBACK_LINES override (#1010)"
run_fragment_test test_panes_snapshot_multi_session_scrollback "panes_snapshot: with two live golems, only the session whose scrollback holds the bar is labelled a form (#1010)"
run_fragment_test test_pane_multi_question_form_stale_bar_in_scrollback "pane_is_multi_question_form: a stale tab bar in scrollback labels a later fork a form — pinned asymmetry (#1010)"
run_fragment_test test_pane_multi_question_form_scrollback_session "pane_is_multi_question_form: the scrollback read targets the session it was given (#1010)"
run_fragment_test test_pane_multi_question_form_scrollback_malformed_knob "pane_is_multi_question_form: a malformed GOLEM_PANE_SCROLLBACK_LINES falls back to the default (#1010)"
run_fragment_test test_pane_multi_question_form_scrollback_fallback "pane_is_multi_question_form: no session / no tmux / empty capture fall back to the visible pane (#1010)"
