...
export async function run() {
  ...
  eq(
    ...
    JSON.stringify(["scope-drift"]),
    "route=cheap: scope-drift is the ONLY dimension that runs",
  );
  ...
  for (const dropped of ["security", "correctness", "tests", "conventions", "decomposition"]) {
    ok(
      !cheapNames.includes(dropped),
      `route=cheap: the ${dropped} dimension is dropped (nothing for it to read in a doc/config-only diff)`,
    );
  }
  ...
  eq(
    ...
    "route=full: every dimension runs, exactly as before #550",
  );
