...
run_fragment_test test_launch_plugin_probe_timeout_is_bounded_refusal "golem-launch: a hanging plugin probe is bounded and refuses (#946)"
run_fragment_test test_launch_plugin_check_escape_hatch "golem-launch: GOLEM_SKIP_PLUGIN_CHECK downgrades the refusal to a warning (#946)"
run_fragment_test test_launch_plugin_probe_absent_skips_silently "golem-launch: an absent plugin probe skips silently (#946)"
run_fragment_test test_print_plugin_absent_warns_but_emits "golem-launch: print warns but still emits the line (#946)"
run_fragment_test test_plugin_guard_message_namespaces_commands "golem-launch: the plugin-guard refusal namespaces its slash-commands (#946)"
run_fragment_test test_preflight_plugin_absent_warns_without_changing_exit "golem-launch: preflight warns without changing its own exit code (#946)"
run_fragment_test test_plugin_probe_honors_marketplace_override "golem-launch: GOLEM_MARKETPLACE reaches the probe (#946)"
...
run_fragment_test test_worktree_new_credential_helper_preserves_different_existing "worktree-new: an operator-configured helper is preserved, not overwritten (#877)"
run_fragment_test test_worktree_new_credential_helper_identical_existing_is_silent "worktree-new: a byte-identical re-seed is a clean silent no-op (#877)"
...
run_fragment_test test_worktree_rm_quarantines_a_wedged_worktree "worktree-rm: a wedged worktree is quarantined so the issue-N path is freed (#936)"
run_fragment_test test_worktree_rm_quarantine_preserves_contents "worktree-rm: the quarantined tree's contents survive — rename, not delete (#936)"
run_fragment_test test_worktree_rm_two_quarantines_are_siblings_not_nested "worktree-rm: two quarantines land as siblings, never nested (#936)"
...
run_fragment_test test_worktree_rm_named_issue_prefix_routes_to_issue_mode "worktree-rm: the issue-N directory spelling routes to ISSUE mode (#1005 review)"
run_fragment_test test_worktree_rm_named_merge_gate_ignores_a_same_named_tag "worktree-rm: the merge gate reads refs/heads, not a same-named tag (#1005 review)"
run_fragment_test test_worktree_rm_named_unresolvable_base_ref_keeps_branch "worktree-rm: an unresolvable base ref fails CLOSED, keeping the branch (#1005 review)"
run_fragment_test test_worktree_rm_named_base_ref_resolves_local_branch "worktree-rm: a local-branch base resolves via refs/heads (#1005 c2)"
...
run_fragment_test test_liveness_background_capable_turn_is_indeterminate "golem-transcript-liveness: a background-capable turn-end is indeterminate, never idle (#890)"
run_fragment_test test_liveness_all_background_tools_are_indeterminate "golem-transcript-liveness: Workflow/Monitor/Bash each arm the indeterminate arm (#890)"
run_fragment_test test_liveness_ordinary_tool_turn_end_still_idle "golem-transcript-liveness: an ordinary-tool turn-end is still idle (#890)"
run_fragment_test test_liveness_no_tool_call_turn_end_still_idle "golem-transcript-liveness: a no-tool-call turn-end is still idle (#890)"
run_fragment_test test_liveness_trailing_text_does_not_mask_background_tool "golem-transcript-liveness: trailing text records do not mask the background tool call (#890)"
...
run_fragment_test test_liveness_open_registration_upgrades_to_background "golem-transcript-liveness: an open registration upgrades indeterminate to background (#949)"
run_fragment_test test_liveness_unregistered_background_turn_stays_indeterminate "golem-transcript-liveness: the same turn with nothing registered stays indeterminate (#949)"
run_fragment_test test_liveness_registry_does_not_break_ordinary_idle "golem-transcript-liveness: an empty registry does not upgrade a genuine idle (#949)"
run_fragment_test test_liveness_aged_out_registration_does_not_pin_background "golem-transcript-liveness: an aged-out registration cannot pin background forever (#949)"
run_fragment_test test_liveness_mode3_no_transcript_with_registration_is_background "golem-transcript-liveness: Mode 3 — no transcript + open item is background (#949)"
run_fragment_test test_liveness_mode3_no_transcript_no_registration_stays_indeterminate "golem-transcript-liveness: Mode 3 — no transcript, nothing open stays indeterminate (#949)"
run_fragment_test test_liveness_background_across_both_env_knobs "golem-transcript-liveness: background resolves across BOTH env knobs (#949)"
run_fragment_test test_work_register_opens_item "golem-work: register opens an item and prints id= as key=value (#949)"
run_fragment_test test_work_complete_closes_item "golem-work: complete closes the item with that id (#949)"
run_fragment_test test_work_complete_unknown_id_is_idempotent "golem-work: completing an unknown id is an idempotent no-op (#949)"
run_fragment_test test_work_multiple_items_counted_independently "golem-work: items are counted and closed independently (#949)"
run_fragment_test test_work_dead_pid_is_reaped "golem-work: bound 1 — an entry whose pid is gone is reaped on read (#949)"
run_fragment_test test_work_live_pid_is_not_reaped "golem-work: bound 1 — an entry whose pid is alive is kept (#949)"
run_fragment_test test_work_aged_out_entry_is_reaped "golem-work: bound 2 — a leaked entry cannot pin a golem to working (#949)"
run_fragment_test test_work_max_age_is_env_overridable "golem-work: bound 2 — GOLEM_WORK_MAX_AGE is env-overridable (#949)"
run_fragment_test test_work_per_entry_max_age_wins "golem-work: bound 2 — a per-entry max_age beats the ambient default (#949)"
run_fragment_test test_work_absent_registry_counts_zero "golem-work: bound 3 — an absent registry counts zero, exit 0 (#949)"
run_fragment_test test_work_empty_registry_counts_zero "golem-work: bound 3 — an empty registry counts zero, exit 0 (#949)"
run_fragment_test test_work_tolerates_malformed_lines "golem-work: a torn append is skipped, not fatal (#949)"
run_fragment_test test_work_list_uses_dash_for_absent_pid "golem-work: an absent pid is the '-' sentinel, so no TSV column collapses (#949)"
run_fragment_test test_work_nojq_write_is_spec_valid_json "golem-work: the no-jq writer emits spec-valid JSON, checked by python3 not jq (#949)"
run_fragment_test test_work_nojq_read_matches_jq_read "golem-work: the no-jq reader reports the same ITEMS as the jq reader, not just the same count (#949)"
run_fragment_test test_work_nojq_bounds_match_jq_bounds "golem-work: the staleness bounds hold in the no-jq reader too (#949)"
run_fragment_test test_work_rejects_zero_and_padded_pid "golem-work: --pid 0/00/000/007 are all rejected numerically (#949)"
run_fragment_test test_work_rejects_zero_and_padded_max_age "golem-work: --max-age 0/00/007 are all rejected numerically (#949)"
run_fragment_test test_work_accepts_canonical_numeric_values "golem-work: canonical numeric values are still accepted (#949)"
run_fragment_test test_work_rejects_non_numeric_pid "golem-work: a non-numeric or negative pid is rejected (#949)"
run_fragment_test test_work_observer_reads_subject_across_both_knobs "golem-work: an observer reads the subject across BOTH env knobs (#949)"
run_fragment_test test_work_writer_and_observer_agree_on_the_same_file "golem-work: writer and observer resolve the same registry file (#949)"
run_fragment_test test_work_observer_ignores_ambient_golem_id "golem-work: a --worktree read ignores an ambient \$GOLEM_ID (#949)"
run_fragment_test test_work_rejects_invalid_kind "golem-work: an unknown kind is rejected (#949)"
run_fragment_test test_work_rejects_traversal_golem_id "golem-work: a traversal golem id is rejected before it becomes a path (#949)"
run_fragment_test test_work_unknown_subcommand_fails_loud "golem-work: an unknown subcommand fails loud (#949)"
run_fragment_test test_work_dangling_flag_refuses "golem-work: a dangling flag refuses instead of collapsing to a default (#949)"
run_fragment_test test_work_count_stays_fail_soft_on_runtime_conditions "golem-work: count stays fail-soft on RUNTIME conditions (#949)"
run_fragment_test test_work_list_observer_flags "golem-work: list's observer flags are exercised directly (#949)"
run_fragment_test test_work_complete_rejects_malformed_id "golem-work: complete rejects a malformed / traversal / duplicate id (#949)"
run_fragment_test test_work_list_rejects_positional_argument "golem-work: list rejects a stray positional argument (#949)"
run_fragment_test test_work_compact_truncates_when_empty "golem-work: the registry FILE is truncated once nothing is open (#949)"
run_fragment_test test_work_status_dir_flag_and_precedence "golem-work: --status-dir works standalone and wins over --worktree (#949)"
...
run_fragment_test test_mode_verify_text_retries_second_enter "golem-mode-check: verify-text sends a second Enter when the first does not submit (#974)"
run_fragment_test test_mode_verify_text_splits_payload_from_submit "golem-mode-check: verify-text sends payload and submit as separate send-keys calls (#974)"
run_fragment_test test_mode_verify_text_unsubmitted_fails_loud "golem-mode-check: verify-text fails loud on a composer that never empties (#974)"
run_fragment_test test_mode_verify_send_blind_to_unsubmitted_text "golem-mode-check: verify-send's pane-changed predicate cannot see unsubmitted text (#974)"
run_fragment_test test_mode_verify_text_dash_payload_is_literal "golem-mode-check: verify-text relays a dash-leading directive literally (#974)"
run_fragment_test test_mode_verify_text_refuses_occupied_composer "golem-mode-check: verify-text refuses to relay onto an occupied composer (#974)"
run_fragment_test test_mode_verify_text_requires_single_text_arg "golem-mode-check: verify-text refuses an unquoted multi-word directive (#974)"
run_fragment_test test_mode_verify_text_empty_directive_exits_2 "golem-mode-check: verify-text refuses an empty directive (#974)"
run_fragment_test test_mode_verify_text_invalid_id_exits_2 "golem-mode-check: verify-text refuses a malformed golem id (#974)"
